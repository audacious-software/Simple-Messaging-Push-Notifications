...
def check_apple_push_setup(app_configs, **kwargs): # pylint: disable=unused-argument
    ...
    if hasattr(settings, 'SIMPLE_MESSAGING_WEB_PRIVATE_KEY') is False:
        error = Warning('SIMPLE_MESSAGING_WEB_SUBJECT parameter not defined', hint='Update configuration to include SIMPLE_MESSAGING_WEB_SUBJECT (web notification subject).', obj=None, id='simple_messaging_push_notifications.W009')
        errors.append(error)
